<template>
    <div class="vue-datepicker" :id="id">
      <!--@click.stop,阻止事件冒泡,取消阻止事件冒泡，点击多个日历，会依次消失-->
        <input @click="show=!show" :value="current | dateFormat" type="text" readonly>
...
<script>
    export default {
        ...
        created() {
            this.transform(this.momentTime);
            this.complete();
            // 获得年份列表： 1900-2100
        ...
        methods: {
            ...
            */
            ...
            // 更改选中时间并向父组件派发事件
            complete() {
                // 注册父组件的传过来的picked事件。三个参数: 年，月，日
                this.$emit('picked', this.select.year, this.select.month, this.select.date,this.id);
                this.current = {
                    year: this.select.year,
                    month: this.select.month,
                    date: this.select.date
